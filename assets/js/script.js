jQuery(document).ready(function ($) {
  ...
  $(".uvd-main-wrapper").on(
    "contextmenu",
    "video, .uvd-download-btn, .uvd-video-preview-wrapper",
    function (e) {
      e.preventDefault();
      return false;
    },
  ...
  $("#uvd-fetch-btn").on("click", function () {
    ...
    $.post(
      uvd_ajax.ajax_url,
      {
        ...
        url: btoa(url), // Base64 encode to prevent Firewall from blocking external URLs in POST
      },
      function (response) {
        if (response.success) {
      ...
      $("#uvd-error-container")
        .html('<p style="margin:0;">Network error. Please try again.</p>')
        .fadeIn();
      btn.prop("disabled", false);
      text.show();
      loader.hide();
  ...
  function renderResult(data) {
    ...
    if (data.hd) {
      ...
      html +=
        '<button type="button" class="uvd-download-btn uvd-download-hd hd" data-url="' +
        data.hd +
        '" data-filename="' +
        hdFilename +
        '" data-type="hd">';
      html +=
        '<svg style="width:20px;height:20px;margin-right:8px;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3"/></svg>';
      html += "Download HD</button>";
    }
    if (data.sd) {
      var sdFilename = baseFilename + "-sd.mp4";
      html +=
        '<button type="button" class="uvd-download-btn uvd-download-sd" data-url="' +
        data.sd +
        '" data-filename="' +
        sdFilename +
        '" data-type="sd">';
    ...
    videoWrapper.on("click", function () {
      if (video.paused) {
        ...
        videoWrapper
          .find(".uvd-video-play-pause-overlay")
          .html('<div class="uvd-pause-icon"></div>');
      } else {
        video.pause();
    ...
    $(".uvd-download-btn").on("click", function () {
      ...
      var shouldShowAd =
        (type === "hd" && uvd_ajax.enable_ads_hd) ||
        (type === "sd" && uvd_ajax.enable_ads_sd);
